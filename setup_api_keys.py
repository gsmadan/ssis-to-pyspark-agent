...
def setup_api_keys():
    ...
    print(f"  OpenAI: {'✅ Set' if existing_keys.get('OPENAI_API_KEY') else '❌ Not set'}")
    print(f"  Gemini: {'✅ Set' if existing_keys.get('GEMINI_API_KEY') else '❌ Not set'}")
    print()
    ...
    if choice == '4':
        print("✅ Skipping API key setup. Using rule-based conversion only.")
        return
    
    if not new_keys:
        print("❌ No API keys provided. Using rule-based conversion only.")
        return
